...
export function FloatingButtons() {
  ...
  return (
    <>
      <a href="https://wa.me/919876543210?text=Hi%20Arroyo%20Engineering%2C%20I%20would%20like%20a%20quote." target="_blank" rel="noopener noreferrer"
...
export function QuotePopupHost() {
  const { open, setOpen } = useQuote();
  const pathname = usePathname();
  if (!open || pathname?.startsWith('/admin')) return null;
  return (
    <div className="fixed inset-0 z-[90] bg-black/85 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setOpen(false)}>
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <button onClick={() => setOpen(false)} className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/70 hover:text-white"><X className="w-4 h-4" /></button>
        ...
        <a href="/contact" onClick={() => setOpen(false)} className="mt-6 inline-flex items-center justify-center gap-2 w-full px-6 py-4 rounded-full bg-gradient-to-r from-red-600 to-red-500 text-white font-semibold">
          Fill Quote Form
        </a>
